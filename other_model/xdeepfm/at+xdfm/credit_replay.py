...
if __name__ == "__main__":
    ...
    path = os.path.join(path, 'at+xdfm_credit.pth')
    print(path)
    data, label = dataset.get_credit_card()
    print(data.shape)
    credit = env.CreditEnv(15)
    actions = [374, 185, 80, 77, 360, 273, 108, 167, 336, 71, 240, 0, 110, 102, 360, 162, 114, 8, 240, 80]
    actions = set(actions)
    sparse_features = []
    ...
    model.fit(train_model_input, y_train.values, batch_size=256, epochs=60, verbose=2, validation_split=0.2)
