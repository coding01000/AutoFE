...
class Action(object):
    def __init__(self, data, bounds):
        ...
        self.action_dim = self.feature_nums * self.action_num + len(self.combinations) * 3
    ...
    def step(self, _action):
        ...
        if not is_combinations:
            ...
            group = int((_action - self.feature_nums * self.action_num) / len(self.combinations))
            tmp = (_action - self.feature_nums * self.action_num + 1) % len(self.combinations)
            action[0] = self.combinations[tmp][0]
            action[1] = self.combinations[tmp][1]
        ...
        if _action not in self.processed:
            if not is_combinations:
                ...
                tmp = group_operator[group](self.data, c1, c2)
...
class BankEnv(object):
    def __init__(self, bounds):
        data, self.label = dataset.get_bank()
